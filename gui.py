...
class Gui():
    ...
    def create_frames(self, window):
        ...
        frm_title_border = tk.Frame(
            master=window, bd=10, height=160, bg="black")
        frm_title = tk.Frame(
            master=frm_title_border,
            bd=30,
            height=100,
            bg="#99d8ea")
        ...
        if True:
            _ = tk.Label(
                master=frm_title,
                text="Information Provenance for Fake News",
                font=(
                    "MS Sans Serif",
                    30,
                    "bold"),
        ...
        if True:
            frm_main_right_border = tk.Frame(
                master=frm_main, bg="Black", bd=10)
            # hides some code (makes things look nicer <3)
            frm_main_right_border.pack(
                fill=tk.BOTH, side=tk.RIGHT, expand=True)
        ...
        frm_url_border = tk.Frame(
            master=frm_main_left,
            bd=10,
            height=130,
            bg="black")
    ...
    def setup_suggested_page_frames(self, frm_main_right, number_of_articles):
        for i in range(number_of_articles):
            ...
            self.frm_suggest_lst_border.append(
                tk.Frame(master=frm_main_right, height=30, bd=5))
            self.frm_suggest_lst.append(
                tk.Frame(
                    master=self.frm_suggest_lst_border[i],
                    bg="Light gray"))
            self.frm_suggest_lst_border[i].pack(
                fill=tk.BOTH, side=tk.TOP, expand=True)
            self.frm_suggest_lst[i].pack(
                fill=tk.BOTH, side=tk.TOP, expand=True)
